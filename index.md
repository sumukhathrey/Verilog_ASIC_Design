...
module seq_1001_moore (input din, clk, rst,
  ...
  always @ (*) begin
    case (state)
      ...
      S4: next_state = din ? S1 : S0; // This transition for non-overlaping sequence detector (If uncommented, comment the next line)
      //S4: next_state = din ? S1 : S2; // This transition for overlaping sequence detector (If uncommented, comment the previous line)
      default: next_state = S0;
    endcase
  end
...
```verilog
module seq_1001_mealy (input din, clk, rst,
                output reg dout);
  ...
  always @ (*) begin
    case (state)
      ...
      S1: next_state = din ? S1 : S2;
      S2: next_state = din ? S1 : S3;
      S3: next_state = S0;// This transition for non-overlaping sequence detector (If uncommented, comment the next line)
